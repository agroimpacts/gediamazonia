...
overlapping cacao fields. Study area is visualized with ggplot to overlay 
geometries effectivley and manual control colors and theme. plot is displayed,
then saved to working folder. 
...
rh level 100 level points clipped by study area displayed over the park, buffer, 
and cacao zone. Map displays in output and saves to working folder
```{r}
rh_points <- ggplot() + geom_sf(data = studyarea$np, color = "grey", fill = "grey70") + 
  ...
  geom_sf(data = level2a_SA, aes(color = rh100)) + ggtitle("Relative Height lvl 100 Distribution")#level 2A rh 100 for study area
  #geom_sf(data = level2b_SA, aes(color = pai)) #level 2A pai for study area
rh_points
ggsave(paste0(working_fold,"/rh_points.png"), rh_points, width = 5, height = 4, dpi = 300)
```

PAI points clipped by study area displayed over the park, buffer, 
and cacao zone. Map displays in output and saves to working folder. 
```{r}
pai_points <- ggplot() + geom_sf(data = studyarea$np, color = "grey", fill = "grey70") + 
  ...
  #geom_sf(data = level2a, aes(color = rh75)) #level 2A relative height 75
  #geom_sf(data = level2a, aes(color = rh100)) #level 2A relative height 100
  geom_sf(data = level2b_SA, aes(color = pai)) + ggtitle("Plant Area Index Distribution")#level 2B plant area index 
pai_points

ggsave(paste0(working_fold,"/pai_points.png"), pai_points, width = 5, height = 4, dpi = 300)
```
...
Additionall preprocessing is necessary for creating boxplots of zones as 
...
road_class <- gedi_ss$distclass %>% as_data_frame(.) %>% mutate(distances = value)
