...
partnership with NASA and JPL. the goal is to get an idea of what GEDI can 
measure regarding canopy cover and relative height metrics. 
LiDAR is a useful tool in mapping canopy coverage.
tradionally done by drones, this project uses LiDAR data collected from the GEDI
system in orbit over the study area. GEDI provides points in a larger footprint 
then traditional drone/aerial platforms. So what is GEDI?

GEDI is a full-waveform lidar instrument which launched on December 5th, 2018. It currently is the highest resolution and densest sampling of any lidar ever put into orbit. It has a horizontal accuracy within +/- 9m, and minimizes off-nadir pointing to less than 6 degrees to avoid inaccuracy. From GEDI waveforms, four types of vegetation structure information can be extracted: surface topography, canopy height metrics, canopy cover metrics, and vertical structure metrics. The raw waveforms and derived products are available online. These products include Geolocated waveforms (L1B), Elevation and Height Metrics (L2A), Canopy Cover Fraction and Leaf Area Index (L2B), and footprint-level above-ground biomass (L4A). Each of these is available at 25 meter diameter resolution. There are also grided forms at 1 km resolution.
...
## Package Installation and library loading. rGEDI package created by Carlos 
Alberto Silva, Caio Hamamura, Ruben Valbuena, Steven Hancock, Adrian Cardil, 
Eben N. Broadbent, Danilo R. A. de Almeida, Celso H. L. Silva Junior 
and Carine Klauberg provides several useful tools for preprocessing, analysis, 
and visualizations. 
...
## Set Area of Interest and Date Range for creating a bounding box, later used
to clip GEDI data
...
!!IF ACQUIRING DATA FOR THE FIRST TIME IN THIS PROJECT
UNCOMMENT DOWNLOAD CODE BELOW!!
...
# Convert Data into a Spatial Data Frame and Export it as a Shapefile. We must 
remove all NA values in the coordinate fields and convert shotnumber to character
before converting. 
...
## Level 2B: Get Plant Area Index and Plant Area Volume Density. Plant Area Index
and volume density are exported as shapefiles and are available for furhter
analysis. 
...
We used basic sf tools to read in the converted GEDI shapefiles, project to the
AOI reference system, and clip based on the AOI bounding box. These clipped GEDI
levels will next be used to estimate plant area index and relative height
metrics based on distance from roads. 
