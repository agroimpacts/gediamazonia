...
st_write(park, dsn = file.path(getwd(), "park.shp"), delete_layer = TRUE)
...
leaflet() %>%
  addCircleMarkers(level2BPAVDProfile_clip_gb$lon_lowestmode,
                   level2BPAVDProfile_clip_gb$lat_lowestmode,
                   radius = 1,
                   opacity = 1,
                   color = "green")  %>%
...
rhlevels %>% 
  ...
  xlab(NULL) + scale_fill_manual(values = c("Dark Green", "white", "light blue")) + ggtitle("Relative Height Metrics")
```
